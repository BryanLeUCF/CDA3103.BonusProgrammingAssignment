...
int Least_recently_used(int num_set){
  ...
  else if(cache[set][2].valid == 0){
    counter = 2;
  }
  else if(cache[set][3].valid == 0){
    counter = 3;
  }
  ...
  if(cache[set][1].LRU > cache[set][2].LRU){
    counter = 2;
  }
  else{
    counter = 1;
  }
  if(cache[set][1].LRU > cache[set][3].LRU){
    lru = 3;
  }
  else{
    lru = 1;
  ...
  if(cache[set][0].LRU > cache[set][2].LRU){
    lru = 2;
  }
  else{
    lru = 0;
  }
  if(cache[set][0].LRU > cache[set][3].LRU){
    lru = 3;
  }
  else{
    lru = 0;
...
int associative(int input){
  ...
  if(tagged[set][1] == tag_value && cache[set][1].valid){
    cache[set][1].LRU = 0;
    return 1;
  }else if(tagged[set][0] == tag_value && cache[set][0].valid){
    cache[set][0].LRU = 0;
    return 1;
   else if(tagged[set][2] == tag_value && cache[set][2].valid){
    cache[set][2].LRU = 0;
    return 1;
  }else if(tagged[set][3] == tag_value && cache[set][3].valid){
    cache[set][3].LRU = 0;
    return 1;
  }
    else{
